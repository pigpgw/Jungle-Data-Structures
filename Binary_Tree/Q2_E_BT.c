...

{
    ...
    if (node->left != NULL) left_height = maxHeight(node->left) + 1;
    if (node->right != NULL) right_height = maxHeight(node->right) + 1; 
    return height = left_height > right_height ? left_height : right_height;
}
